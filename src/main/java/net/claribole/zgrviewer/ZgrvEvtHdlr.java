 ...
 *   $Id: ZgrvEvtHdlr.java 988 2007-11-30 11:09:52Z epietrig $
 */
...
import net.guha.apps.DisplayStructure;
    ...
    public void click1(ViewPanel v, int mod, int jpx, int jpy, int clickNumber, MouseEvent e) {
        ...
        if (toolPaletteIsActive) {
            if (v.lastGlyphEntered() != null) {
                grMngr.tp.selectButton((VImage) v.lastGlyphEntered());
            }
        } else {
                ...
                if (clickNumber == 2) {
                    ...
                    if (mod == SHIFT_MOD) {
                        ...
                        if (ConfigManager.chemicalData == null) {
                            JOptionPane.showMessageDialog(grMngr.mainView.getFrame(), "Must load chemical data to show structures",
                                    "SALI Viewer Error", JOptionPane.ERROR_MESSAGE);
                            return;
                        }
                        if (o instanceof LNode) {
                            LNode node = (LNode) o;
                            DisplayStructure.showNodeStructure(node.getTitle());
                        } else if (o instanceof LEdge) {
                            LEdge edge = (LEdge) o;
                            DisplayStructure.showEdgeStructures(edge.getHead().getTitle(), edge.getTail().getTitle());
                        }
                    } else {
