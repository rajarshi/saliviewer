 ...
 *
 *   $Id: ConfigManager.java 707 2007-06-28 08:09:04Z epietrig $
 */
...
import net.guha.apps.scurve.SALICurve;
...
public class ConfigManager {
    ...
    static final String saliURI = "http://cheminfo.informatics.indiana.edu/~rguha/code/java/salivis";

    static final String MAIN_TITLE = "SALI Viewer";
    ...
    /* SALI related stuff                   */
    /****************************************/
    public static Hashtable chemicalData;
    public static SmilesParser smilesParser;
    public static boolean skipBadEntries = false;
    public static boolean firstColumnIsSmiles = true;
    public static boolean smallerIsBetter = true;
    public static boolean highlightNonSS = false;
    public static int depictionX = 200;
    public static int depictionY = 200;
    public static int fpLength = 1024;
    public static int fpDepth = 6;
    public static String fpType = "standard";
    public static SALIPair[] saliPairs = null; // this won't get serialized
    public static String[] headers = null; // this won't get serialized
    public static SALICurve saliCurve = null;
    ...
    void loadConfig() {
        if (cfgFile.exists()) {
            ...
            try {
                ...
                try {
                    ConfigManager.m_PrjDir = new File(e.getElementsByTagNameNS(ConfigManager.zgrvURI, "graphDir").item(0).getFirstChild().getNodeValue());
                }
                catch (Exception ex) {
                }
                try {
                    ConfigManager.m_DotPath = new File(e.getElementsByTagNameNS(ConfigManager.zgrvURI, "dot").item(0).getFirstChild().getNodeValue());
                }
                catch (Exception ex) {
                }
                try {
                    ConfigManager.m_NeatoPath = new File(e.getElementsByTagNameNS(ConfigManager.zgrvURI, "neato").item(0).getFirstChild().getNodeValue());
                }
                catch (Exception ex) {
                }
                try {
                    ConfigManager.m_CircoPath = new File(e.getElementsByTagNameNS(ConfigManager.zgrvURI, "circo").item(0).getFirstChild().getNodeValue());
                }
                catch (Exception ex) {
                }
                try {
                    ConfigManager.m_TwopiPath = new File(e.getElementsByTagNameNS(ConfigManager.zgrvURI, "twopi").item(0).getFirstChild().getNodeValue());
                }
                catch (Exception ex) {
                }
                try {
                    ConfigManager.m_GraphVizFontDir = new File(e.getElementsByTagNameNS(ConfigManager.zgrvURI, "graphvizFontDir").item(0).getFirstChild().getNodeValue());
                }
                catch (Exception ex) {
                }
                // sali setting
                try {
                    e = (Element) (rt.getElementsByTagNameNS(ConfigManager.saliURI, "depiction")).item(0);
                    ConfigManager.depictionX = Integer.parseInt(e.getAttribute("width"));
                    ConfigManager.depictionY = Integer.parseInt(e.getAttribute("height"));
                    ConfigManager.highlightNonSS = e.getAttribute("highlight").equals("true");

                    e = (Element) (rt.getElementsByTagNameNS(ConfigManager.saliURI, "fingerprint")).item(0);
                    ConfigManager.fpDepth = Integer.parseInt(e.getAttribute("depth"));
                    ConfigManager.fpLength = Integer.parseInt(e.getAttribute("length"));
                    ConfigManager.fpType = e.getAttribute("type");

                    e = (Element) (rt.getElementsByTagNameNS(ConfigManager.saliURI, "fileinput")).item(0);                    
                    ConfigManager.skipBadEntries = e.getAttribute("skipBadEntries").equals("true");
                    ConfigManager.firstColumnIsSmiles = e.getAttribute("firstColumnSmiles").equals("true");

                    e = (Element) (rt.getElementsByTagNameNS(ConfigManager.saliURI, "salinetwork")).item(0);
                    ConfigManager.smallerIsBetter = e.getAttribute("smallerIsBetter").equals("true");
    ...
    void saveConfig() {        
        ...
        rt.setAttribute("xmlns:sali", ConfigManager.saliURI);
        ...
        // sali settings
        Element sali = cfg.createElementNS(ConfigManager.saliURI, "sali:depiction");
        sali.setAttribute("width", String.valueOf(ConfigManager.depictionX));
        sali.setAttribute("height", String.valueOf(ConfigManager.depictionY));
        sali.setAttribute("highlight", String.valueOf(ConfigManager.highlightNonSS));

        rt.appendChild(sali);
        sali = cfg.createElementNS(ConfigManager.saliURI, "sali:fingerprint");
        sali.setAttribute("depth", String.valueOf(ConfigManager.fpDepth));
        sali.setAttribute("length", String.valueOf(ConfigManager.fpLength));
        sali.setAttribute("type", ConfigManager.fpType);
        rt.appendChild(sali);
        sali = cfg.createElementNS(ConfigManager.saliURI, "sali:fileinput");
        sali.setAttribute("skipBadEntries", String.valueOf(ConfigManager.skipBadEntries));
        sali.setAttribute("firstColumnSmiles", String.valueOf(ConfigManager.firstColumnIsSmiles));
        rt.appendChild(sali);
        sali = cfg.createElementNS(ConfigManager.saliURI, "sali:salinetwork");
        sali.setAttribute("smallerIsBetter", String.valueOf(ConfigManager.smallerIsBetter));        
        rt.appendChild(sali);
...
// 	consts.setAttribute("graphOrient",ConfigManager.GRAPH_ORIENTATION);
    ...
    void rememberCommandLine(String cmdLine) {
        ...
        }  //we limit the list to COMMAND_LIMIT elements
    }
