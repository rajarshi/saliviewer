 ...
 *   $Id: PrefWindow.java 576 2007-03-29 18:32:53Z epietrig $
 */
...
class PrefWindow extends JFrame implements ActionListener, MouseListener {
    ...
    JSpinner fpLengthSpinner, fpDepthSpinner;
    JComboBox fpTypeBox;
    JCheckBox highlightNonSS;
    JCheckBox skipBadEntry;
    JCheckBox firstColSmiles;
    JCheckBox smallerIsBetter;
    ...
    PrefWindow(ZGRViewer app, GraphicsManager gm) {
        ...
        // sali panel
        // first we get a font for our section border titles
        Font borderFont = new Font("Sans Serif", Font.BOLD, 10);

        JPanel saliPanel = new JPanel();
        saliPanel.setLayout(new BoxLayout(saliPanel, BoxLayout.PAGE_AXIS));

        GridBagLayout gbl1 = new GridBagLayout();
        GridBagConstraints gbc1 = new GridBagConstraints();
        gbc1.fill = GridBagConstraints.HORIZONTAL;
        gbc1.anchor = GridBagConstraints.WEST;
        JPanel depictionPanel = new JPanel(gbl1);

        SpinnerModel spinnerModelX = new SpinnerNumberModel(ConfigManager.depictionX, 50, 1000, 10);
        SpinnerModel spinnerModelY = new SpinnerNumberModel(ConfigManager.depictionY, 50, 1000, 10);
        spinnerx = new JSpinner(spinnerModelX);
        spinnery = new JSpinner(spinnerModelY);
        JLabel labelx = new JLabel("Depiction Width");
        JLabel labely = new JLabel("Depiction Height");

        buildConstraints(gbc1, 0, 0, 1, 1, 100, 10);
        gbl1.setConstraints(labelx, gbc1);
        depictionPanel.add(labelx);
        labelx.setLabelFor(spinnerx);

        buildConstraints(gbc1, 1, 0, 1, 1, 100, 10);
        gbl1.setConstraints(spinnerx, gbc1);
        depictionPanel.add(spinnerx);

        buildConstraints(gbc1, 0, 1, 1, 1, 100, 10);
        gbl1.setConstraints(labely, gbc1);
        depictionPanel.add(labely);
        labelx.setLabelFor(spinnery);

        buildConstraints(gbc1, 1, 1, 1, 1, 100, 10);
        gbl1.setConstraints(spinnery, gbc1);
        depictionPanel.add(spinnery);

        highlightNonSS = new JCheckBox("Highlight differences");
        highlightNonSS.setSelected(ConfigManager.highlightNonSS);
        highlightNonSS.addActionListener(this);
        buildConstraints(gbc1, 0, 2, 1, 1, 100, 10);
        gbl1.setConstraints(highlightNonSS, gbc1);
        depictionPanel.add(highlightNonSS);

        TitledBorder depictionBorder = BorderFactory.createTitledBorder("Depiction");
        depictionBorder.setTitleJustification(TitledBorder.LEFT);
        depictionBorder.setTitleFont(borderFont);
        depictionPanel.setBorder(depictionBorder);
        ...
        JPanel ioPanel = new JPanel(gbl2);
        skipBadEntry = new JCheckBox("Skip bad file entries");
        skipBadEntry.setSelected(ConfigManager.skipBadEntries);
        skipBadEntry.addActionListener(this);
        buildConstraints(gbc2, 0, 0, 1, 1, 100, 10);
        gbl2.setConstraints(skipBadEntry, gbc2);
        ioPanel.add(skipBadEntry);
        firstColSmiles = new JCheckBox("First column is SMILES");
        firstColSmiles.setSelected(ConfigManager.firstColumnIsSmiles);
        firstColSmiles.addActionListener(this);
        buildConstraints(gbc2, 0, 1, 1, 1, 100, 10);
        gbl2.setConstraints(firstColSmiles, gbc2);
        ioPanel.add(firstColSmiles);
        depictionBorder = BorderFactory.createTitledBorder("I/O Options");
        depictionBorder.setTitleJustification(TitledBorder.LEFT);
        depictionBorder.setTitleFont(borderFont);
        ioPanel.setBorder(depictionBorder);
        ...
        fpTypeBox = new JComboBox(new String[]{"standard", "extended", "graph"});
        fpTypeBox.setSelectedItem(ConfigManager.fpType);
        fpTypeBox.setEditable(false);
        fpTypeBox.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                JComboBox cb = (JComboBox) e.getSource();
                String item = (String) cb.getSelectedItem();
                ConfigManager.fpType = item;                
            }
        });

        SpinnerModel fpLengthSpinnerModel = new SpinnerNumberModel(ConfigManager.fpLength, 64, 4096, 128);
        SpinnerModel fpDepthSpinnerModel = new SpinnerNumberModel(ConfigManager.fpDepth, 3, 10, 1);
        fpLengthSpinner = new JSpinner(fpLengthSpinnerModel);
        fpDepthSpinner = new JSpinner(fpDepthSpinnerModel);
        JLabel label1 = new JLabel("Fingerprint length");
        JLabel label2 = new JLabel("Fingerprint depth");
        JLabel label3 = new JLabel("Fingerprint type");
        JPanel calcPanel = new JPanel(gbl3);

        buildConstraints(gbc3, 0, 0, 1, 1, 100, 10);
        gbl3.setConstraints(label3, gbc3);
        calcPanel.add(label3);
        label1.setLabelFor(fpTypeBox);
        buildConstraints(gbc3, 1, 0, 1, 1, 100, 10);
        gbl3.setConstraints(fpTypeBox, gbc3);
        calcPanel.add(fpTypeBox);

        buildConstraints(gbc3, 0, 1, 1, 1, 100, 10);
        gbl3.setConstraints(label1, gbc3);
        calcPanel.add(label1);
        label1.setLabelFor(fpLengthSpinner);
        buildConstraints(gbc3, 1, 1, 1, 1, 100, 10);
        gbl3.setConstraints(fpLengthSpinner, gbc3);
        calcPanel.add(fpLengthSpinner);

        buildConstraints(gbc3, 0, 2, 1, 1, 100, 10);
        gbl3.setConstraints(label2, gbc3);
        calcPanel.add(label2);
        label2.setLabelFor(fpDepthSpinner);
        buildConstraints(gbc3, 1, 2, 1, 1, 100, 10);
        gbl3.setConstraints(fpDepthSpinner, gbc3);
        calcPanel.add(fpDepthSpinner);

        smallerIsBetter = new JCheckBox("Lower activity is better");
        smallerIsBetter.setSelected(ConfigManager.smallerIsBetter);
        smallerIsBetter.addActionListener(this);
        buildConstraints(gbc3, 0, 3, 1, 1, 100, 10);
        gbl3.setConstraints(smallerIsBetter, gbc3);
        calcPanel.add(smallerIsBetter);

        depictionBorder = BorderFactory.createTitledBorder("Network Options");
        depictionBorder.setTitleJustification(TitledBorder.LEFT);
        depictionBorder.setTitleFont(borderFont);
        calcPanel.setBorder(depictionBorder);

        saliPanel.add(calcPanel);
        saliPanel.add(depictionPanel);
        saliPanel.add(ioPanel);
        tabbedPane.addTab("SALI.", saliPanel);
        ...
        if (ConfigManager.autoDetectBrowser) {
            detectBrowserBt.doClick();
        } //select and fire event
        else {
            specifyBrowserBt.doClick();
        } //so that fields get enabled/disabled as is approriate
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (o == browseTmpDirBt) {//tmp directory browse button
            ...
            ConfigManager.DELETE_TEMP_FILES = cb1.isSelected();
        } else if (o == skipBadEntry) {
            ConfigManager.skipBadEntries = skipBadEntry.isSelected();
        } else if (o == firstColSmiles) {
            ConfigManager.firstColumnIsSmiles = firstColSmiles.isSelected();
        } else if (o == smallerIsBetter) {
            ConfigManager.smallerIsBetter = smallerIsBetter.isSelected();
        } else if (o == highlightNonSS) {
            ConfigManager.highlightNonSS = highlightNonSS.isSelected();
            ...
            if (antialiascb.isSelected()) {
                javax.swing.JOptionPane.showMessageDialog(this, Messages.antialiasingWarning);
            }
    ...
    void updateVars() {
        ...
        ConfigManager.depictionX = ((Integer) spinnerx.getValue()).intValue();
        ConfigManager.depictionY = ((Integer) spinnery.getValue()).intValue();

        ConfigManager.fpLength = ((Integer) fpLengthSpinner.getValue()).intValue();
        ConfigManager.fpDepth = ((Integer) fpDepthSpinner.getValue()).intValue();
    }
